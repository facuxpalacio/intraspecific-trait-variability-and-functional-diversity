...
library(cati) # T-statistics, Rao's entropy decomposition and ITV simulations
...
# Single trait data visualization {#trait_plot}
...
```
From trait distributions, it seems that species do not strongly vary between them, but there is high variability within the same species.
...
For this particular case, this reveals that not accounting for ITV largely underestimates functional diversity. This is because intraspecific variability is much more higher than interspecific variance.
...
Ck <- NULL
for(i in 1:ncomms){
Ck[[i]] <- matrix(0, nrow = ncomms, ncol = indxcomm)
Ck[[i]][i, ] <- 1
}
Cind <- do.call(cbind, Ck)
colnames(Cind) <- rownames(traits)

# Build hypervolumes for each community. Due to computheadation times, I use only 10 random points per individual to simulate kernels, but should be increased to a minimum of 100 in practice for a good description of the underlying trait distribution.
hvlist <- kernel.build(comm = Cind, trait = traits, axes = 2, distance = "gower", method = "gaussian", abund = TRUE, samples.per.point = 10)


hvlist <- list()
for(i in 1:ncomms){
  comm <- matrix(1, nrow = 1, ncol = nsp)
  hvlist[[i]] <- kernel.build(comm = comm, trait = trait_matrix[[i]][, -1], axes = 2, distance = "gower", method = "gaussian", abund = FALSE, samples.per.point = 10)
}

kernel_hv <- matrix(NA, nrow = ncomms, ncol = 3)
  for(i in 1:ncomms){
    kernel_hv[i, 1] <- kernel.alpha(hvlist[[i]])
    kernel_hv[i, 2] <- kernel.evenness(hvlist[[i]])
    kernel_hv[i, 3] <- kernel.dispersion(hvlist[[i]], func = "divergence")
  }
colnames(kernel_hv) <- c("hv_richness", "hv_regularity", "hv_divergence")
...
```{r kernel_plots}
kernel_df <- data.frame(kernel_hv, TOP_TED, dendroFD, TPD_FRic =  TPD_FD$communities$FRichness,
                        TPD_FEve = TPD_FD$communities$FEvenness, 
                        TPD_FDiv = TPD_FD$communities$FDivergence)
...
Again, hypervolume metrics are pretty uncorrelated to those above, because they work differently. However, as they are probabilistic in nature and conceptually similar, the highest correlation is found between kernel hypervolume and TPD functional richness.
...
```{r MVNH plots}
mvnh_df <- cbind(MVNH_comms, dendroFD, TOP = TOP_TED$TOP, kernel_hv, 
