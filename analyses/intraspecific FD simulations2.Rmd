...
min_trait_comm <- seq(20, 0, length = nsim)
max_trait_comm <- seq(30, 50, length = nsim)
...
mean_comm_sim$cv_comm <- sort(rep(seq(0.5, 1, length = nsim), nsim)) # Parameter to change
...
cv_sp <- seq(0.05, 0.5, length = nsim) # Parameter to change
```
...
# Individual trait values per species (aca me quede)
list_names <- 1:(nsim*nsim)
...
subtrait <- data.frame(trait = trait_matrix$V1, comm = trait_matrix$comm, sp = trait_matrix$sp)
...
subtrait <- data.frame(trait = trait_matrix$V10, comm = trait_matrix$comm, sp = trait_matrix$sp)
plot_comm_trait(trait_matrix = subtrait, trait = subtrait$trait, comm_label = subtrait$comm, sp_label = subtrait$sp, species = T)

# Low external filtering, high internal filtering
subtrait <- data.frame(trait = trait_matrix$V91, comm = trait_matrix$comm, sp = trait_matrix$sp)
plot_comm_trait(trait_matrix = subtrait, trait = subtrait$trait, comm_label = subtrait$comm, sp_label = subtrait$sp, species = T)

# Low external filtering, low internal filtering
subtrait <- data.frame(trait = trait_matrix$V100, comm = trait_matrix$comm, sp = trait_matrix$sp)
plot_comm_trait(trait_matrix = subtrait, trait = subtrait$trait, comm_label = subtrait$comm, sp_label = subtrait$sp, species = T)
