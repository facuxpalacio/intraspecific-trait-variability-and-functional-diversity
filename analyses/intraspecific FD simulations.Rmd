...
ni1 <- 2
cv_intra_sp <- seq(0.01, 1, length = ni1)

# Coefficient of variation of intracommunity trait distributions
nj1 <- 2
cv_intra_com <- seq(0.01, 1, length = nj1)

# Strength of internal filtering
ni2 <- 2
Int_Filter_Strength <- seq(0, 100, length = nj2)
...
comms <- list(comm_sim[[1]], comm_sim[[101]], comm_sim[[505]], comm_sim[[625]])
...
colnames(var_comps) <- c("Intra_comm_perc", "Between_comm_perc", "Within_species_perc")
rownames(var_comps) <- c("Low_int_low_ext", "Low_int_high_ext", "High_int_low_ext", "High_int_high_ext")
var_comps

# High internal, High external
ggplot(comm_sim[[625]]$data, aes(x = trait1, fill = as.factor(com))) + geom_density(alpha = 0.4)

# Low internal, high external
ggplot(comm_sim[[505]]$data, aes(x = trait1, fill = as.factor(com))) + geom_density(alpha = 0.4)

# High internal, low external
ggplot(comm_sim[[101]]$data, aes(x = trait1, fill = as.factor(com))) + geom_density(alpha = 0.4)

# Low internal, Low external
ggplot(comm_sim[[1]]$data, aes(x = trait1, fill = as.factor(com))) + geom_density(alpha = 0.4)
